...
const wallet = new ethers.Wallet(Buffer.from(mnemonic, "hex"));
...
let checkLiq = async () => {
  ...
  if (pairAddressx !== null && pairAddressx !== undefined) {
    // console.log("pairAddress.toString().indexOf('')", pairAddress.toString().indexOf('0x0000000000000'));
...
let buyAction = async () => {
  ...
  try {
    ...
    const amountOutMin = amounts[1].mul(`${100 - data.Slippage}`).div(`100`);
    ...
    console.log("amountIn", amountIn, amountIn.toString() / 1e18);
    console.log("amountOutMin", amountOutMin, amountOutMin.toString() / 1e18);
    console.log("[tokenIn, tokenOut]", [tokenIn, tokenOut]);
    console.log("data.recipient", data.recipient);
    console.log("Date.now() + 1000 * 60 * 5", Date.now() + 1000 * 60 * 5);
