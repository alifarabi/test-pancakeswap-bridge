...
class PancakeTradeBot {
    bsc: string;
    panRouterContractAddress: string;
    panAbi: string;
  ...
  constructor(addresses) {
      ...
      const { address: admin } = this.web3.eth.accounts.wallet.add(process.env.PRIVATE_KEY)
  }

  public async setToken() {
    const [PAIR, CURRENCY] = await Promise.all(
      [this.addresses.PAIR, this.addresses.CURRENCY].map(tokenAddress => {
        new Token(
          ChainId.MAINNET,
          tokenAddress,
          18
    ...
    // const pair = await Fetcher.fetchPairData(PAIR, CURRENCY, this.provider)
...
const init = async () => {
  const tardeBot = new PancakeTradeBot(addresses)

  await tardeBot.setToken()
